create database ElderlyCareSupport collate SQL_Latin1_General_CP1_CI_AS
go

grant connect on database :: ElderlyCareSupport to dbo
go

grant view any column encryption key definition, view any column master key definition on database :: ElderlyCareSupport to [public]
go

create table dbo.ElderCareAccount
(
    Id          bigint identity (1000, 1)
        constraint UQ_ElderCareAccount_Id
            unique,
    FirstName   nvarchar(200) not null,
    LastName    nvarchar(200),
    Email       nvarchar(200) not null
        constraint PK__ElderCar__3214EC07457AF218
            primary key,
...
create table dbo.FEE_CONFIGURATION
(
    FEE_ID      numeric(17) identity,
    FEE_NAME    varchar(50) not null,
    FEE_AMOUNT  numeric(17) not null,
...
(
    ...
    TaskDescription varchar(700),
    StartDate       datetime     not null,
    EndDate         datetime     not null,
    TaskStatusId    int
        references dbo.TaskStatus,
    ElderlyPersonId bigint
        references dbo.ElderCareAccount (Id),
    CreatedDate     datetime default getdate(),
    UpdatedDate     datetime default getdate(),
    TaskCategoryId  int
        constraint FK_TaskDetails_TaskCategory
            references dbo.TaskCategory
)
go

create table dbo.TaskCategoryLink
(
    TaskId         int not null
        references dbo.Task,
    TaskCategoryId int not null
        references dbo.TaskCategory,
    primary key (TaskId, TaskCategoryId)
)
go

create table dbo.TaskHistory
(
    TaskHistoryId    int identity
        primary key,
    TaskId           int
        references dbo.Task,
    StatusId         int
...
create table dbo.VolunteerAccount
(
    Id              bigint identity (1000, 1),
    FirstName       nvarchar(200) not null,
    LastName        nvarchar(200),
    Email           nvarchar(200) not null,
    Password        nvarchar(200) not null,
    ConfirmPassword nvarchar(200) not null,
    PhoneNumber     bigint        not null,
    Gender          nvarchar(200) not null,
    Address         nvarchar(200) not null,
    City            nvarchar(200) not null,
...
CREATE   PROC Get_Fee_Details
AS
BEGIN
    SELECT FEE_ID, FEE_NAME, FEE_AMOUNT FROM dbo.FEE_CONFIGURATION;
END;

    EXEC Get_Fee_Details;

DECLARE @time DATETIME= GETDATE();
SELECT FEE_ID, FEE_NAME, FEE_AMOUNT FROM dbo.FEE_CONFIGURATION;
    WAITFOR DELAY '00:00:05';
DECLARE @time1 DATETIME= GETDATE();
SELECT DATEDIFF( MICROSECOND, @time, @time1) AS EXCECUTIONTIMEINSECONDS;
go
